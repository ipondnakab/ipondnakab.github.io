import { ProjectExperience } from "@/interfaces/work-experience";

export const OUTSOURCE_PROJECTS: ProjectExperience[] = [
  {
    title: "Pa Yai Ha Mor (Took Grandma to see the Doctor)",
    description: `A mobile
        application to assist the elderly in visiting hospitals. Develop
        using ReactNative, NodeJS, and Firebase which got an honorable
        mention at NSC 2021 (National Software Contest)`,
  },
  {
    title: "SMT",
    description: `Web application market management`,
  },
  {
    title: "Pettinee",
    description: `Platform for consulting veterinarians`,
  },
  {
    title: "VMKKU",
    description: `Veterinarians consultation platform for The Faculty
        of Veterinary Medicine, Khon Kaen University.`,
  },
  {
    title: "REAL CONTROL TECHNOLOGY CO., LTD.",
    description: `Corporate Website and OKR
        (Objective and Key Results) Platform`,
  },
];
